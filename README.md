...
> brightness** setting; and **opt-in accelerometer tilt tuning** — roll the
> Cardputer **ADV** to move through a linear transponder's passband (off by
> default, and shown as *n/a* on boards without the sensor).
> See **[RELEASE_NOTES_0.9.17.md](RELEASE_NOTES_0.9.17.md)**.
...
> automatically near closest approach**, plus an optional **TCA-tapered predictive
> lead** to mask CAT latency. On 450° rotators, azimuth tracking now **looks ahead
> and pre-commits to the overlap band before a north crossing** for shortest-path
> slewing. Also: **per-satellite calibrations can be hand-authored** on the SD card
> (`/CardSat/calib.txt`), and those files are now comment-friendly.
> See **[RELEASE_NOTES_0.9.15.md](RELEASE_NOTES_0.9.15.md)**.

> **New in v0.9.14:** reliable **offline transponder caching**. "Cache all
> transponders" (`a` on the Update screen) now caches the full catalog in small
> batches across **automatic reboots** — each reboot gives a fresh network
> connection, so the whole set caches successfully even on a weak Wi-Fi link,
> with **no satellite skipped** (a sat that fails is re-attempted after the next
> reboot). The **Update** action now also runs space-weather and weather fetches
> in sequence, and NOAA's slow-first-response feeds get a longer connect window.
> See **[RELEASE_NOTES_0.9.14.md](RELEASE_NOTES_0.9.14.md)**.
