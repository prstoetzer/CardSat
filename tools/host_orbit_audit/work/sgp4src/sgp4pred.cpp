...
void Sgp4::findsat(unsigned long unixt){
  findsat(getJulianFromUnix(unixt));
}
...
bool Sgp4::nextpass( passinfo* passdata, int itterations, bool direc, double minimumElevation){
    ...
    if (isdaylight)	{(*passdata).vismax = sgp4_daylight;}
    ...
    if (isdaylight)	{(*passdata).visstart = sgp4_daylight;}
    ...
    if (isdaylight)	{(*passdata).visstop = sgp4_daylight;}
    ...
    if ((*passdata).visstop == sgp4_daylight && (*passdata).visstart == sgp4_daylight)	{(*passdata).sight = sgp4_daylight;}
    ...
    (*passdata).vistransit = sgp4_daylight;
	}
	else {
		if (sgn(startphi)>sgn(stopphi)) {
    ...
    if (isdaylight)	{(*passdata).vistransit = sgp4_daylight;}
...
bool Sgp4::initpredpoint( unsigned long unixt, double startelevation){
  return initpredpoint( getJulianFromUnix(unixt), startelevation);
}
