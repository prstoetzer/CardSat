...
class Sgp4 {
    ...
    double sunoffset;  //Min elevation sun for sgp4_daylight in radials
  ...
  public:
    ...
    void setsunrise(double degrees);   //change the elevation that the sun needs to make it sgp4_daylight

    void findsat(double jdI);     //find satellite position from julian date
    void findsat(unsigned long);  //find satellite position from unixt time
    ...
    bool initpredpoint( unsigned long unixt, double startelevation); // from unixt time
